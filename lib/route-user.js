...
export default function routeUser(user) {
  ...
  if (user?.milestones?.isLmiFlow) {
    localStorage.setItem("offer", "lmi");
  }
  if (user?.milestones?.isLmiFlow && user?.milestones?.lmiInfoCompleted) {
    localStorage.setItem("offer", "lmiCompleted");
  }
